  ...
  @try {
    ...
    if ([collection isKindOfClass:NSArray.class]) {
      ...
      for (id x in collection) {
        [result addObject:f(x)];
      }
    } else if ([collection isKindOfClass:NSDictionary.class]) {
      result = [NSMutableDictionary dictionaryWithCapacity:[collection count]];
      for (id key in collection) {
        [result setObject:f([collection objectForKey:key]) forKey:key];
      }
    }
    return result;
...
+ (instancetype)fromJSONDictionary:(NSDictionary *)dict {
  @try {
    if (dict) {
      return [[BOASDKManifest alloc] initWithJSONDictionary:dict];
    }
  } @catch (NSException *exception) {
    BOFLogDebug(@"%@:%@", BOA_DEBUG, exception);
  }
...
+ (instancetype)fromJSONDictionary:(NSDictionary *)dict {
  @try {
    if (dict) {
      return [[BOASDKVariable alloc] initWithJSONDictionary:dict];
    }
  } @catch (NSException *exception) {
    BOFLogDebug(@"%@:%@", BOA_DEBUG, exception);
  }
