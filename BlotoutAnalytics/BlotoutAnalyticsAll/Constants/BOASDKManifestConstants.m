//
//  BOASDKManifestConstants.m
//  BlotoutAnalytics
//
//  Created by Pawan Singh Jat on 21/09/20.
//  Copyright © 2020 Blotout. All rights reserved.
//


#import <Foundation/Foundation.h>
#import "BOASDKManifestConstants.h"

NSString * const Server_Port = @"Server_Port";
NSString * const Api_Endpoint = @"Api_Endpoint";
NSString * const Event_CodifiedMergeCounter = @"Event_Codified_Mergecounter";
NSString * const Event_Offline_Interval = @"Event_Offline_Interval";
NSString * const Event_GEOLocationGrain = @"Event_Geolocation_Grain";
NSString * const Event_DeviceInfoGrain = @"Event_Deviceinfo_Grain";
NSString * const Event_SystemMergeCounter = @"Event_System_Mergecounter";
NSString * const Event_PushThreshold_Interval = @"Event_Push_Interval";
NSString * const Event_PushThreshold_EventCounter = @"Event_Push_Eventscounter";
NSString * const EVENT_PATH = @"Event_Path";
NSString * const EVENT_RETENTION_PATH = @"Event_Retention_Path";
NSString * const Event_SDK_Manifest_Path = @"SDK_Manifest_Path";
NSString * const Interval_Manifest_Refresh = @"Manifest_Refresh_Interval";
NSString * const Interval_Retry = @"Retry_Interval";
NSString * const Interval_Store_Events = @"Store_Events_Interval";
NSString * const Event_Push_System_Events = @"SDK_Push_System_Events";
NSString * const Event_SDK_Map_User_Id = @"SDK_Map_User_Id_Events";
NSString * const Event_SDK_Behaviour_Id = @"SDK_Behaviour_Events";
NSString * const Event_Push_PII_Events = @"Event_Push_PII_Events";
NSString * const Event_Push_PHI_Events = @"Event_Push_PHI_Events";
NSString * const Event_PHI_Public_Key = @"PHI_Public_Key";
NSString * const Event_PII_Public_Key = @"PII_Public_Key";
NSString * const License_Expire_Day_Alive = @"License_Expire_Day_Alive";
NSString * const MANIFEST_APP_LAST_UPDATED_TIME = @"Last_Updated_Time";
NSString * const GEO_IP_PATH = @"Geo_Ip_Path";
NSString * const Mode_Deployment = @"Mode_Deployment";
NSString * const Customer_Type = @"Customer_Type";
