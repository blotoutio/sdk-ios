...
class BOAUtilities {
    ...
    class func getDeviceId() -> String? {
        ...
        do{
            ...
            if deviceUUID != nil && (deviceUUID?.count ?? 0) > 0 {
                ...
                deviceId = guidString ?? BOAUtilities.getUUIDString(fromString: stringBuilder)
    ...
    class func getUUIDString(from uuidStr: String?) -> String? {
        ...
        do{
            ...
            let tempUniqueID = CFUUIDCreateString(kCFAllocatorDefault, uuidRef) as String
    ...
    @objc class func systemName() -> String? {
        do{
            ...
            if UIDevice.current.responds(to: #selector(UIDevice.systemName)) {
    ...
    @objc class func systemVersion() -> String? {
        do{
            ...
            if UIDevice.current.responds(to: #selector(UIDevice.systemVersion)) {
            ...
            if ProcessInfo.processInfo.responds(to: #selector(ProcessInfo.operatingSystemVersionString)) {
    ...
    class func getUserBirthTimeStamp() -> NSNumber? {
        let timeStamp = NSNumber(value: 0)
        do{
            let analyticsRootUD = BOFUserDefaults(for: BO_ANALYTICS_ROOT_USER_DEFAULTS_KEY)
    ...
    class func setUserBirthTimeStamp(_ timeStamp: NSNumber?) {
       do{
            var analyticsRootUD = BOFUserDefaults(for: BO_ANALYTICS_ROOT_USER_DEFAULTS_KEY)
    ...
    class func data(fromPlist plist: Any) -> Data? {
        do{
            ...
            if let error = error {
                BOFLogDebug(frmt: "Unable to serialize data from plist object", args: error.localizedDescription, plist)
            }
            return data
        } catch {
            return nil
    ...
    class func plist(from data: Data) -> Any? {
        ...
        if let error = error {
            BOFLogDebug("Unable to parse plist from data %@", error.localizedDescription)
        }
