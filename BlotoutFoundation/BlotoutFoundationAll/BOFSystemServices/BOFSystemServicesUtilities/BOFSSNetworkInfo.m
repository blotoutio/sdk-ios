...
@interface BOFRoute_Info : NSObject
{
    struct sockaddr     m_addrs[RTAX_MAX];
    struct bo_rt_msghdr2   m_rtm;
    int                 m_len;      /* length of the sockaddr array */
}
- initWithRtm: (struct bo_rt_msghdr2*) rtm;

+ (NSMutableArray*) getRoutes;
+ (nullable BOFRoute_Info*) getRoute:(struct bo_rt_msghdr2 *)rtm;
...
-initWithRtm: (struct bo_rt_msghdr2*) rtm
{
    ...
    memcpy(&(m_rtm), rtm, sizeof(struct bo_rt_msghdr2));
...
+ (NSMutableArray*) getRoutes
{
    ...
    BOFRoute_Info* route = nil;
    
    size_t len;
    int mib[6];
    char *buf;
    register struct bo_rt_msghdr2 *rtm;
...
+ (nullable BOFRoute_Info*) getRoute:(struct bo_rt_msghdr2 *)rtm
{
    ...
    if(rtm->rtm_addrs & RTA_DST)
    {
        ...
        {
            route = [[BOFRoute_Info alloc] initWithRtm:rtm];
        }
    }
        ...
        switch(sa->sa_family)
        {
            case AF_INET:
            {
                ...
                if(si->sin_addr.s_addr == INADDR_ANY)
                    ...
                    routeString = [NSString stringWithCString:(char *)inet_ntoa(si->sin_addr) encoding:NSUTF8StringEncoding];
            }
                break;
            ...
            case AF_LINK:
            {
                ...
                if(sdl->sdl_nlen + sdl->sdl_alen + sdl->sdl_slen == 0)
                {
                    ...
                    routeString = [NSString stringWithCString:link_ntoa(sdl) encoding:NSUTF8StringEncoding];
            }
                break;
            ...
            default:
            {
                ...
                else
                {
                    ...
                    routeString = [NSString stringWithCString:a encoding:NSUTF8StringEncoding];
                }
            }
        }
    }
...
@implementation BOFSSNetworkInfo
...
+ (nullable NSString *)externalIPAddress {
    @try {
        ...
        NSString *externalIP = @"";
        //[NSString stringWithContentsOfURL:[NSURL URLWithString:@"https://icanhazip.com/"] encoding:NSUTF8StringEncoding error:&error];
...
+ (nullable NSString *)wiFiRouterAddress {
    ...
    @try {
        ...
        NSMutableArray *routerArray = [BOFRoute_Info getRoutes];
        // Run through the array
        for(int i = 0; i < (int)[routerArray count]; i++)
        {
            // Set the router info
            BOFRoute_Info* router = (BOFRoute_Info*)[routerArray objectAtIndex:i];
